...
const MultiSelectToolbar: React.FC<MultiSelectToolbarProps> = ({
  ...
  if (isMobile && !isMultiSelectMode) {
    return null; // Don't render anything on mobile if not in multi-select mode
  }

  return (
    <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm rounded-2xl">
      <div className="container mx-auto px-4 py-4">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          {!isMultiSelectMode && !isMobile && (
            <div className="flex items-center gap-4 border border-transparent py-2 min-h-[56px]">
              <button
                onClick={onStartMultiSelect}
                className="group flex items-center gap-2 px-8 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg shadow-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-300 whitespace-nowrap flex-shrink-0"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                {session && (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg hover:from-purple-700 hover:to-purple-800 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-purple-300"
                ...
                {session && (
                  <button
                    onClick={onMoveImages}
                    disabled={selectedImagesCount === 0}
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-300"
                    aria-label={`Move ${selectedImagesCount} selected images`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-2m-4-1v5m0 0l3-3m-3 3L9 6m-5 9v4a2 2 0 002 2h12a2 2 0 002-2v-4"></path></svg>
                    <span>Move</span>
                    {selectedImagesCount > 0 && (
                      <span className="bg-blue-800 text-white text-xs px-2 py-0.5 rounded-full font-bold">
                        {selectedImagesCount}
                      </span>
                    )}
                  </button>
